...
public:
	...
	public:
		...
		static inline size_t next_power_of_two(size_t i)
		{
			--i;
			i |= i >> 1;
			i |= i >> 2;
		...
		public:
			...
			void reserve(size_t num_e)
			{
				size_t required_buckets = num_buckets_for_reserve(num_e);
		...
		private:
			...
			size_t num_buckets_for_reserve(size_t num_e) const
			{
				return static_cast<size_t>(std::ceil(num_e / std::min(0.5, static_cast<double>(_max_load_factor))));
			}
			...
			void deallocate_data(EntryPointer begin, size_t num_slots_m1, int8_t mx_lookups)
			{
				if (begin != Entry::empty_default_table())
				{
					AllocatorTraits::deallocate(*this, begin, num_slots_m1 + mx_lookups + 1);
				}
			}
